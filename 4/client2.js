...
const handleClick =  (x, y) =>socket.emit('make_move', [x, y, playerValue] )
...
socket.on('play_game', ([markedPositions, highlighted]) => ReactDOM.render( React.createElement( root, {markedPositions, highlighted} ), document.getElementById('root') ) )


const rowButtons = ({index, row}) => 
{
	row.map( oneElement => React.createElement('span', {onClick: () => handleClick(index, row.indexOf(oneElement))}, oneElement )  )
}
