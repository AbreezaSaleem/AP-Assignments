...
const getSites = (siteInfo, waitFor) => new Promise( (resolve, reject) =>
{
	...
	Promise.all( Object.keys(siteInfo).forEach(key =>
	{
	...
	{	
		console.log('GET SITES RESOLVED! <3')
		resolve()
	})
})


const init = (siteNames, waitFor) =>
{
...
const goTolink = (outgoingLinks, currentSite) => new Promise( (resolve, reject) =>
{
	...
	if (numRequests <= obj.maxRequests && outgoingLinks.length >= 0)
	{
		...
		{		
			...
			{
				console.log('already')
				finalIndex.currentSite.outgoing.push(oneLink) // for your current domain, add these outgoing links
			}
			else
			{
				console.log('ADDING...')
				finalIndex[domainNameOfSite] = {outgoing:[oneLink], incoming: []} 
			}

			if (domainNameOfLink in finalIndex)
			{
				console.log('already')
				finalIndex.domainNameOfLink.incoming.push(currentSite)
			}
			else
			{
				console.log('ADDING...')
				finalIndex[domainNameOfLink] = {outgoing:[], incoming: [currentSite]}
			}*/
