...
const readSite = (link, waitFor) => new Promise ( (resolve, reject) => 
{
	...
	{
		numRequests++
		console.log('Site ' + link + ' has been read!')
...
const parseSites = (parser, response, currentSite) => {
	...
	parsed.on('finish', () => parseDone(count++, currentSite))
...
const getSites = (siteInfo, waitFor) =>
{
...
fileRead.then(() => getSites(siteInfo, 10000))
	...
	if (numRequests <= obj.maxRequests)
	{
		...
		{		
			...
			if (domainName != null)
			{
				Object.keys(siteInfo).forEach(key => 
				{
					newKey = key.replace('https:/', '').replace('http:/', '').replace('/', '').replace('/', '')
		...
		getSites(siteInfo, obj.minFetchInterval)		
		/*console.log('\n')
